                            ...
                            tabItem(tabName = "Map",
                                    fluidPage(
                                        fluidRow(column(6,
                                                 ...
                                                 #                    choices = levels(Shooting_data$VIC_AGE_GROUP),
                                                 #                    options = list(`actions-box` = TRUE),
                                                 #                    multiple = TRUE))),
                                        fluidRow(column(6,
...
server <- function(input, output) {
    ...
    #                                  BORO %in% input$boro1 & VIC_SEX %in% input$sex & 
    #                                  VIC_RACE %in% input$race & VIC_AGE_GROUP %in% input$age)
    # })
    # 
    # output$mapMarker <- renderLeaflet({
    #     leaflet(st, options = leafletOptions(minZoom = 0, maxZoom = 24)) %>%
    #         addTiles('http://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}.png') %>%
    #         setView(lng = -73.95, lat = 40.72, zoom = 11) %>%
    #         addCircleMarkers(lng = ~filteredData9()$Longitude, lat = ~filteredData9()$Latitude,
    #                          popup = content, color = ~filteredData9()$VIC_SEX,
    ...
    #     Shooting_data %>% filter(OCCUR_DATE >= input$start_date & OCCUR_DATE <= input$end_date) %>%
    #         filter(BORO %in% ifelse(is.null(input$boro1) == F, input$boro1, Shooting_data$BORO)) %>%
    #         filter(VIC_SEX %in% ifelse(is.null(input$sex) == F, input$sex, Shooting_data$VIC_SEX)) %>%
    #         filter(VIC_RACE %in% ifelse(is.null(input$race) == F, input$race, Shooting_data$VIC_RACE)) %>%
    #         filter(VIC_AGE_GROUP %in% ifelse(is.null(input$age) == F, input$age, Shooting_data$VIC_AGE_GROUP))
    # })
    # 
    # output$mapMarker <- renderLeaflet({
    #     leaflet(st, options = leafletOptions(minZoom = 0, maxZoom = 18)) %>%
    #         addTiles('http://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}.png') %>%
    #         setView(lng = -73.95, lat = 40.72, zoom = 11) %>%
    #         addCircleMarkers(lng = ~filteredData8()$Longitude, lat = ~filteredData8()$Latitude,
    #                          popup = content, clusterOptions = markerClusterOptions())
    # })
